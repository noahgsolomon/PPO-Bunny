...
export default function Page() {
  ...
  const playingAnimation = useSpring({
    opacity: gameState.state === 'PLAYING' ? 1 : 0,
    transform: gameState.state === 'PLAYING' ? 'translateY(0)' : 'translateY(100%)',
    config: webConfig.wobbly,
  })
    ...
    <div>
      ...
      >
        ...
        <div className='flex flex-row gap-2'>
          <Button onClick={() => gameState.setState('PLAYING')} size='lg'>
            Play
          </Button>
      ...
      <animated.div style={playingAnimation} className='absolute z-10 left-16 top-12 flex flex-row gap-2 items-center'>
        <Avatar
